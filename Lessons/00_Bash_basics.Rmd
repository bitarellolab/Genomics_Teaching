...
Note: a directory is what you ordinarily call a "folder" in a computer. For example, on my mac I have a `Documents` folder where I have a `B216` folder. Those are both directories, or subdirectories. The ultimate directory is the one that contains all the subdirectories! On Mac/Unix machines, that's given by `/`.
...
*For the questions below, use the [answer sheet template](https://docs.google.com/document/d/1qO2RiXaoLY19L2nsPT7oDLESz2P_UDeuQxnQJWs826o/edit?usp=sharing) provided to type your answers.*
...
Now let's use the tail command. Repeat the above command replacing `head` with `tail`:

```{bash, echo=T, eval=F}
#type your command in your terminal 
```

**Q9) What did the `tail` command do?**
...
Hmm, what are all these numbers? This would be a good time to check the documentation for the `wc` program. Type this into your terminal to get some information about what `wc` does:

```{bash, echo=T, eval=F}
wc --help
```

This help page will show you more information than you realistically need right now, like a list of options you can use to tell the program more specifically what you want. 

The most important bit of information right now is near the top of the help output, where it describes what is printed when you run `wc` on a file or files.

Note: the `--help` flag might not work on your laptop, due to some obscure differences in software. On Mac, you can use `man wc` (man = "manual") to get the same info.

**Q10) What did the `wc` command do?**
...
* Press `q` when you're ready to stop looking at the file.

* Congrats! You looked at a giant file without having to open it!


When you're done here, go to the left sidebar and double click on the file called `01_fastqe.html`.
