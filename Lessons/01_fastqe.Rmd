...
First, `cd` into the directory we'll be using: `~/filestoshare/fastq/gartersnake`.

Next, list what's in this directory:
...
```         
 ...
 (Note: You will need to use all of these elements in your command):
...
You should now have 3 new files in your current folder:

-   .html file (this is your QC report)
-   .json file (ignore this for now)
-   trimmed fastq file (out.Male5_oral1.fastq)

Click on the fastp.html file in the left sidebar pane inside `filestoshare/fastq/gartersnake` on the left to examine this report.

*Note: Some of the plots might not show up correctly in the browser inside the mybinder.org environment. You can also open the file in your normal browser by right-clicking the file in the left sidebar and selecting "Open in new browser tab".*
...
FASTQE Official Webpage. <https://fastqe.com/>

FASTP Github Repository. <https://github.com/OpenGene/fastp>

FASTQ files and Quality Control. <https://datacarpentry.org/wrangling-genomics/02-quality-control/index.html>
